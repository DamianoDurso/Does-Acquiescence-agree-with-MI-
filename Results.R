...
simdes <- subset(simdes, simdes$DIFARS == F)
simdes <- simdes[,-c(8)]
...
Fit.NoARS_Int_chi <- NA
Fit.NoARS_Int_df <- NA
Fit.NoARS_Int_RMSEA <- NA
Fit.NoARS_Int_cfi <- NA
Fit.NoARS_Int_comb <- NA
...
Results.scale <- cbind(simdes, NoARS_Conf_RMSEA, NoARS_Conf_cfi,
                       ...
                       NoARS_Int_chi, NoARS_Int_RMSEA, NoARS_Int_cfi,NoARS_Int_comb,
                       ...
                       ARS_Int_chi, ARS_Int_RMSEA, ARS_Int_cfi, ARS_Int_comb,
                       ARS_vs_noARS, Conv.Conf.ARS, Conv.Thr.ARS, Conv.Load.ARS, Conv.Int.ARS, 
                       Conv.Conf, Conv.Thr, Conv.Load, Conv.Int
)
...
Results.scale.diff <- cbind(simdes,
                       ...
                       NoARS_Int_chi, NoARS_Int_RMSEA, NoARS_Int_cfi,
                       ARS_Thr_chi, ARS_Thr_RMSEA, ARS_Thr_cfi, 
                       ARS_Load_chi, ARS_Load_RMSEA, ARS_Load_cfi,
                       ARS_Int_chi, ARS_Int_RMSEA, ARS_Int_cfi
)
...
Results.scale.mean <- cbind(simdes, NoARS_Conf_chi, NoARS_Conf_df, NoARS_Conf_RMSEA, NoARS_Conf_cfi,
                       ...
                       NoARS_Int_chi,NoARS_Int_df, NoARS_Int_RMSEA, NoARS_Int_cfi,
                       ...
                       ARS_Int_chi,ARS_Int_df, ARS_Int_RMSEA, ARS_Int_cfi
)
...
#  if (any(k == (c(11,12,18,23,24,35,36)))){
  rem <- NA
  Scale_level.models_temp <- list()
    for(i in 1:150){
    if(all(Scale_level.models[[i]]$error == 'none')){
    rem[i] <- i      
...
#str(Scale_level.models[[1]])

if(k == 23){
Scale_level.models <- Scale_level.models[-c(12,13)] 
}
...
for(i in 1:100){
  ...
  NoARS_Thr_comb[i] <- all((Scale_level.models[[i]]$Comparative.fit$NoARSModels$value$`Pr(>Chisq)`[2] < .05) & 
                           (any(Scale_level.models[[i]]$Comparative.fit$NoARSModels[[4]]["rmsea.scaled"] > .01,
                                Scale_level.models[[i]]$Comparative.fit$NoARSModels[[4]]["cfi.scaled"] < -.01)))
  ...
  NoARS_Load_comb[i] <- all((Scale_level.models[[i]]$Comparative.fit$NoARSModels$value$`Pr(>Chisq)`[3] < .05) & 
                             (any(Scale_level.models[[i]]$Comparative.fit$NoARSModels[[7]]["rmsea.scaled"] > .01,
                                  Scale_level.models[[i]]$Comparative.fit$NoARSModels[[7]]["cfi.scaled"] < -.01)))
  
  NoARS_Int_chi[i] <- Scale_level.models[[i]]$Comparative.fit$NoARSModels$value$`Pr(>Chisq)`[4]# < .05
  NoARS_Int_df[i] <- Scale_level.models[[i]]$fit.measures$Int.fit["df.scaled"]# < .05
  NoARS_Int_RMSEA[i] <- Scale_level.models[[i]]$Comparative.fit$NoARSModels[[10]]["rmsea.scaled"]# > 0.01
  NoARS_Int_cfi[i] <- Scale_level.models[[i]]$Comparative.fit$NoARSModels[[10]]["cfi.scaled"]# > .01
  NoARS_Int_comb[i] <- all((Scale_level.models[[i]]$Comparative.fit$NoARSModels$value$`Pr(>Chisq)`[4] < .05) & 
                              (any(Scale_level.models[[i]]$Comparative.fit$NoARSModels[[10]]["rmsea.scaled"] > .01,
                                   Scale_level.models[[i]]$Comparative.fit$NoARSModels[[10]]["cfi.scaled"] < -.01)))
  ...
  Fit.NoARS_Int_chi[i] <- Scale_level.models[[i]]$fit.measures$Int.fit["chisq.scaled"]# > 0.01
  Fit.NoARS_Int_df[i] <- Scale_level.models[[i]]$fit.measures$Int.fit["df.scaled"]# > 0.01
  Fit.NoARS_Int_RMSEA[i] <- Scale_level.models[[i]]$fit.measures$Int.fit["rmsea.scaled"]# > 0.01
  Fit.NoARS_Int_cfi[i] <- Scale_level.models[[i]]$fit.measures$Int.fit["cfi.scaled"]# > .01
  ...
  if(any(Scale_level.models[[i]]$Comparative.fit$ARSModels$error[1] != "none" | Scale_level.models[[i]]$Comparative.fit$ARSModels[[4]] != "none" )){
    ...
    ARS_Thr_comb[i] <- all((Scale_level.models[[i]]$Comparative.fit$ARSModels$value$`Pr(>Chisq)`[2] < .05) & 
                             (any(Scale_level.models[[i]]$Comparative.fit$ARSModels[[4]]["rmsea.scaled"] > .01,
                                  Scale_level.models[[i]]$Comparative.fit$ARSModels[[4]]["cfi.scaled"] < -.01)))
  }

  if(any(Scale_level.models[[i]]$Comparative.fit$ARSModels$error[1] != "none" | Scale_level.models[[i]]$Comparative.fit$ARSModels[[7]] != "none")){
  ...
  if(Scale_level.models[[i]]$Comparative.fit$ARSModels$error[1] != "none"){
    ...
    ARS_Load_comb[i] <- all((Scale_level.models[[i]]$Comparative.fit$ARSModels$value$`Pr(>Chisq)`[3] < .05) & 
                              (any(Scale_level.models[[i]]$Comparative.fit$ARSModels[[7]]["rmsea.scaled"] > .01,
                                   Scale_level.models[[i]]$Comparative.fit$ARSModels[[7]]["cfi.scaled"] < -.01)))
  }
  
  if(any(Scale_level.models[[i]]$Comparative.fit$ARSModels$error[1] != "none" | Scale_level.models[[i]]$Comparative.fit$ARSModels[[10]] != "none")){
    ARS_Int_chi[i] <- NA# < .05
  } else {
    ARS_Int_chi[i] <- Scale_level.models[[i]]$Comparative.fit$ARSModels$value$`Pr(>Chisq)`[4]# < .05
  }
  
  ARS_Int_RMSEA[i] <- Scale_level.models[[i]]$Comparative.fit$ARSModels[[10]]["rmsea.scaled"]# > 0.01
  ARS_Int_cfi[i] <- Scale_level.models[[i]]$Comparative.fit$ARSModels[[10]]["cfi.scaled"]# > 0.01
  if(Scale_level.models[[i]]$Comparative.fit$ARSModels$error[1] != "none"){
    ARS_Int_comb[i] <- NA# < .05
  } else {
    ARS_Int_comb[i] <- all((Scale_level.models[[i]]$Comparative.fit$ARSModels$value$`Pr(>Chisq)`[4] < .05) & 
                              (any(Scale_level.models[[i]]$Comparative.fit$ARSModels[[10]]["rmsea.scaled"] > .01,
                                   Scale_level.models[[i]]$Comparative.fit$ARSModels[[10]]["cfi.scaled"] < -.01)))
  }
...
# ARS Absolute fit
  ...
  Fit.ARS_Int_chi[i] <- Scale_level.models[[i]]$fit.measures$Int.fitARS["chisq.scaled"]# > 0.01
  Fit.ARS_Int_df[i] <- Scale_level.models[[i]]$fit.measures$Int.fitARS["df.scaled"]# > 0.01
  Fit.ARS_Int_RMSEA[i] <- Scale_level.models[[i]]$fit.measures$Int.fitARS["rmsea.scaled"]# > 0.01
  Fit.ARS_Int_cfi[i] <- Scale_level.models[[i]]$fit.measures$Int.fitARS["cfi.scaled"]# > 0.01
  ...
  if(is.double(Scale_level.models[[i]]$fit.measures$Int.fitARS)){
    Conv.Int.ARS[i] <- "Yes"
  } else {
    Conv.Int.ARS[i] <- "No"
  }
  ...
  if(is.double(Scale_level.models[[i]]$fit.measures$Int.fit)){
    Conv.Int[i] <- "Yes"
  } else {
    Conv.Int[i] <- "No"
  }
  ...
  Results.scale$NoARS_Conf_RMSEA[k] <-  length(which(NoARS_Conf_RMSEA >= .06))/length(NoARS_Conf_RMSEA)
  ...
  Results.scale$NoARS_Int_chi[k] <- length(which(NoARS_Int_chi < .05))/length(NoARS_Int_chi)
  Results.scale$NoARS_Int_RMSEA[k] <- length(which(NoARS_Int_RMSEA >= .01))/length(NoARS_Int_RMSEA)
  Results.scale$NoARS_Int_cfi[k] <- length(which(NoARS_Int_cfi <= -.01))/length(NoARS_Int_cfi)
  Results.scale$NoARS_Int_comb[k] <- length(which(NoARS_Int_comb == T))/length(NoARS_Int_comb)
  
    
  Results.scale$ARS_Conf_RMSEA[k] <-  length(which(unlist(ARS_Conf_RMSEA) >= .06))/length(unlist(ARS_Conf_RMSEA))
  ...
  Results.scale$ARS_Int_chi[k] <- length(which(unlist(ARS_Int_chi) < .05))/length(unlist(ARS_Int_chi))
  Results.scale$ARS_Int_RMSEA[k] <- length(which(unlist(ARS_Int_RMSEA) >= .01))/length(unlist(ARS_Int_RMSEA))
  Results.scale$ARS_Int_cfi[k] <- length(which(unlist(ARS_Int_cfi) <= -.01))/length(unlist(ARS_Int_cfi))
  Results.scale$ARS_Int_comb[k] <- length(which(unlist(ARS_Int_comb) == T))/length(unlist(ARS_Int_comb))
  ...
  Results.scale.diff$NoARS_Int_chi[k] <- paste0(round(mean(unlist(Fit.NoARS_Int_chi - Fit.NoARS_Load_chi)),3), " (", round(sd(unlist(Fit.NoARS_Int_chi - Fit.NoARS_Load_chi)),3), ")" )
  Results.scale.diff$NoARS_Int_RMSEA[k] <- paste0(format(round(mean(unlist(Fit.NoARS_Int_RMSEA - Fit.NoARS_Load_RMSEA)),3),nsmall=3), " (", format(round(sd(unlist(Fit.NoARS_Int_RMSEA - Fit.NoARS_Load_RMSEA)),3),nsmall=3), ")" )
  Results.scale.diff$NoARS_Int_cfi[k] <- paste0(format(round(mean(unlist(Fit.NoARS_Int_cfi - Fit.NoARS_Load_cfi)),3),nsmall=3), " (", format(round(sd(unlist(Fit.NoARS_Int_cfi - Fit.NoARS_Load_cfi)),3),nsmall=3), ")" )
  ...
  Results.scale.diff$ARS_Int_chi[k] <- paste0(round(mean(unlist(mapply('-',Fit.ARS_Int_chi, Fit.ARS_Load_chi))),3), " (", round(sd(unlist(mapply('-',Fit.ARS_Int_chi, Fit.ARS_Load_chi))),3), ")" )
  Results.scale.diff$ARS_Int_RMSEA[k] <- paste0(format(round(mean(unlist(mapply('-',Fit.ARS_Int_RMSEA, Fit.ARS_Load_RMSEA))),3),nsmall=3), " (", format(round(sd(unlist(mapply('-',Fit.ARS_Int_RMSEA, Fit.ARS_Load_RMSEA))),3),nsmall=3), ")" )
  Results.scale.diff$ARS_Int_cfi[k] <- paste0(format(round(mean(unlist(mapply('-',Fit.ARS_Int_cfi, Fit.ARS_Load_cfi))),3),nsmall=3), " (", format(round(sd(unlist(mapply('-',Fit.ARS_Int_cfi, Fit.ARS_Load_cfi))),3),nsmall=3), ")" )
  
  Results.scale$ARS_vs_noARS[k] <- length(which(ARS_vs_noARS < .05))/100
   
  #Mean goodness-of-fit measures
  Results.scale.mean$NoARS_Conf_chi[k] <-    paste0(format(round(mean(unlist(NoARS_Conf_chi)),1),nsmall=1), " (", format(round(sd(unlist(NoARS_Conf_chi)),1),nsmall=1), ")" )
  Results.scale.mean$NoARS_Conf_df[k] <-    round(mean(NoARS_Conf_df),1)
  ...
  Results.scale.mean$NoARS_Thr_chi[k] <- paste0(round(mean(unlist(Fit.NoARS_Thr_chi)),1), " (", round(sd(unlist(Fit.NoARS_Thr_chi)),1), ")" ) 
  Results.scale.mean$NoARS_Thr_df[k] <- round(mean(NoARS_Thr_df),1)
  ...
  Results.scale.mean$NoARS_Thr_cfi[k] <- paste0(format(round(mean(unlist(Fit.NoARS_Thr_cfi)),3),nsmall=3), " (", format(round(sd(unlist(Fit.NoARS_Thr_cfi)),3),nsmall=3), ")" )
   Results.scale.mean$NoARS_Thr_comb[k] <- paste0(round(mean(unlist(NoARS_Thr_comb)),3), " (", round(sd(unlist(NoARS_Thr_comb)),3), ")" )
  Results.scale.mean$NoARS_Load_chi[k] <- paste0(round(mean(unlist(Fit.NoARS_Load_chi)),1), " (", round(sd(unlist(Fit.NoARS_Load_chi)),1), ")" )
  Results.scale.mean$NoARS_Load_df[k] <- round(mean(NoARS_Load_df),1)
  ...
  Results.scale.mean$NoARS_Int_chi[k] <- paste0(round(mean(unlist(Fit.NoARS_Int_chi)),1), " (", round(sd(unlist(Fit.NoARS_Int_chi)),1), ")" )
  Results.scale.mean$NoARS_Int_df[k] <- round(mean(unlist(NoARS_Int_df)),1)
  Results.scale.mean$NoARS_Int_RMSEA[k] <- paste0(format(round(mean(unlist(Fit.NoARS_Int_RMSEA)),3),nsmall=3), " (", format(round(sd(unlist(Fit.NoARS_Int_RMSEA)),3),nsmall=3), ")" )
  Results.scale.mean$NoARS_Int_cfi[k] <- paste0(format(round(mean(unlist(Fit.NoARS_Int_cfi)),3),nsmall=3), " (", format(round(sd(unlist(Fit.NoARS_Int_cfi)),3),nsmall=3), ")" )
  
  Results.scale.mean$ARS_Conf_chi[k] <-    paste0(format(round(mean(unlist(ARS_Conf_chi)),1),nsmall=1), " (", format(round(sd(unlist(ARS_Conf_chi)),1),nsmall=1), ")" )
  Results.scale.mean$ARS_Conf_df[k] <-    round(mean(unlist(ARS_Conf_df)),1)
  ...
  Results.scale.mean$ARS_Thr_chi[k] <- paste0(round(mean(unlist(Fit.ARS_Thr_chi)),1), " (", round(sd(unlist(Fit.ARS_Thr_chi)),1), ")" ) 
  Results.scale.mean$ARS_Thr_df[k] <-  round(mean(unlist(Fit.ARS_Thr_df)),1)
  ...
  Results.scale.mean$ARS_Thr_cfi[k] <- paste0(format(round(mean(unlist(Fit.ARS_Thr_cfi)),3),nsmall=3), " (", format(round(sd(unlist(Fit.ARS_Thr_cfi)),3),nsmall=3), ")" )
    Results.scale.mean$NoARS_Thr_comb[k] <- paste0(round(mean(unlist(NoARS_Thr_comb)),3), " (", round(sd(unlist(NoARS_Thr_comb)),3), ")" )
  Results.scale.mean$ARS_Load_chi[k] <- paste0(round(mean(unlist(Fit.ARS_Load_chi)),1), " (", round(sd(unlist(Fit.ARS_Load_chi)),1), ")" )
  Results.scale.mean$ARS_Load_df[k] <- round(mean(unlist(Fit.ARS_Load_df)),1)
  ...
  Results.scale.mean$ARS_Load_cfi[k] <- paste0(format(round(mean(unlist(Fit.ARS_Load_cfi)),3),nsmall=3), " (", format(round(sd(unlist(Fit.ARS_Load_cfi)),3),nsmall=3), ")" )
    Results.scale.mean$NoARS_Load_comb[k] <- length(which(NoARS_Load_comb == T))/length(NoARS_Load_comb)
  Results.scale.mean$ARS_Int_chi[k] <- paste0(round(mean(unlist(Fit.ARS_Int_chi)),1), " (", round(sd(unlist(Fit.ARS_Int_chi)),1), ")" )
  Results.scale.mean$ARS_Int_df[k] <- round(mean(unlist(Fit.ARS_Int_df)),1)
  Results.scale.mean$ARS_Int_RMSEA[k] <- paste0(format(round(mean(unlist(Fit.ARS_Int_RMSEA)),3),nsmall=3), " (", format(round(sd(unlist(Fit.ARS_Int_RMSEA)),3),nsmall=3), ")" )
  Results.scale.mean$ARS_Int_cfi[k] <- paste0(format(round(mean(unlist(Fit.ARS_Int_cfi)),3),nsmall=3), " (", format(round(sd(unlist(Fit.ARS_Int_cfi)),3),nsmall=3), ")" )
...
#Convergence results  
  ...
  Results.scale$Conv.Int.ARS[k] <- length(unlist(which(Conv.Int.ARS == "Yes")))/100
  ...
  Results.scale$Conv.Int[k] <- length(unlist(which(Conv.Int == "Yes")))/100
...
Conv.Res_test_null <- cbind(Conv.Res_test_null[,1:7], round(Conv.Res_test_null[,41:44],3))

#ARS Results MI Convergence
Conv.Res_test <- subset(Results.scale, Results.scale$ARS != 0 )
Conv.Res_test_NoARS <- cbind(Conv.Res_test[,1:7], round(Conv.Res_test[,41:44],3))
Conv.Res_test <- cbind(Conv.Res_test[,1:7], round(Conv.Res_test[,37:40],3))
...
Res_test_null <- cbind(Res_test_null[,1:7], round(Res_test_null[,8:35],3))

#ARS Results MI
Res_test <- subset(Results.scale, Results.scale$ARS != 0 )
Res_test <- cbind(Res_test[,1:7], round(Res_test[,8:35],3))
...
Conv.Res_NoArs.fin <- cbind(subset(Conv.Res_NoArs, Conv.Res_NoArs$Nfac==1), subset(Conv.Res_NoArs, Conv.Res_NoArs$Nfac==2)[8:11])
...
Res_1fac_ARS <- rbind(Res_test_small_1fac[order(Res_test_small_1fac$scale, Res_test_small_1fac$sample, Res_test_small_1fac$j),-c(8:21)],
      Res_test_large_1fac[order(Res_test_large_1fac$scale, Res_test_large_1fac$sample, Res_test_large_1fac$j),-c(8:21)])
      
      
Res_1fac_NoARS <- rbind(Res_test_small_1fac[order(Res_test_small_1fac$scale, Res_test_small_1fac$sample, Res_test_small_1fac$j),-c(22:35)],
                    Res_test_large_1fac[order(Res_test_large_1fac$scale, Res_test_large_1fac$sample, Res_test_large_1fac$j),-c(22:35)])

####################################################################################################################################
# 2 FACTORS
Res_2fac_ARS <- rbind(Res_test_small_2fac[order(Res_test_small_2fac$scale, Res_test_small_2fac$sample, Res_test_small_2fac$j),-c(8:21)],
                    Res_test_large_2fac[order(Res_test_large_2fac$scale, Res_test_large_2fac$sample, Res_test_large_2fac$j),-c(8:21)])


Res_2fac_NoARS <- rbind(Res_test_small_2fac[order(Res_test_small_2fac$scale, Res_test_small_2fac$sample, Res_test_small_2fac$j),-c(22:35)],
                    Res_test_large_2fac[order(Res_test_large_2fac$scale, Res_test_large_2fac$sample, Res_test_large_2fac$j),-c(22:35)])


####################################################################################################################################
# Null

Res_test_null.final <- Res_test_null[order(Res_test_null$Nfac, Res_test_null$scale, Res_test_null$sample, Res_test_null$j),c(1:7,8:21)]
...
Mean.Res_test_null <- cbind(Mean.Res_test_null[,1:7], Mean.Res_test_null[,8:23])

Diff.Res_test_null <- subset(Results.scale.diff, Results.scale.diff$ARS == 0 )
Diff.Res_test_null <- cbind(Diff.Res_test_null[,1:7], Diff.Res_test_null[,8:16])

#ARS Results MI
Mean.Res_test <- subset(Results.scale.mean, Results.scale.mean$ARS != 0 )
Mean.Res_test <- cbind(Mean.Res_test[,1:7], Mean.Res_test[,8:39])

Diff.Res_test <- subset(Results.scale.diff, Results.scale.diff$ARS != 0 )
Diff.Res_test <- cbind(Diff.Res_test[,1:7], Diff.Res_test[,8:25])
...
Mean.Res_1fac_ARS <- rbind(Mean.Res_test_small_1fac[order(Mean.Res_test_small_1fac$scale, Mean.Res_test_small_1fac$sample, Mean.Res_test_small_1fac$j),-c(8:23)],
                      Mean.Res_test_large_1fac[order(Mean.Res_test_large_1fac$scale, Mean.Res_test_large_1fac$sample, Mean.Res_test_large_1fac$j),-c(8:23)])


Mean.Res_1fac_NoARS <- rbind(Mean.Res_test_small_1fac[order(Mean.Res_test_small_1fac$scale, Mean.Res_test_small_1fac$sample, Mean.Res_test_small_1fac$j),-c(24:39)],
                        Mean.Res_test_large_1fac[order(Mean.Res_test_large_1fac$scale, Mean.Res_test_large_1fac$sample, Mean.Res_test_large_1fac$j),-c(24:39)])
...
Diff.Res_1fac_ARS <- rbind(Diff.Res_test_small_1fac[order(Diff.Res_test_small_1fac$scale, Diff.Res_test_small_1fac$sample, Diff.Res_test_small_1fac$j),-c(8:16)],
                           Diff.Res_test_large_1fac[order(Diff.Res_test_large_1fac$scale, Diff.Res_test_large_1fac$sample, Diff.Res_test_large_1fac$j),-c(8:16)])


Diff.Res_1fac_NoARS <- rbind(Diff.Res_test_small_1fac[order(Diff.Res_test_small_1fac$scale, Diff.Res_test_small_1fac$sample, Diff.Res_test_small_1fac$j),-c(17:28)],
                             Diff.Res_test_large_1fac[order(Diff.Res_test_large_1fac$scale, Diff.Res_test_large_1fac$sample, Diff.Res_test_large_1fac$j),-c(17:28)])

####################################################################################################################################
# 2 FACTORS
Mean.Res_2fac_ARS <- rbind(Mean.Res_test_small_2fac[order(Mean.Res_test_small_2fac$scale, Mean.Res_test_small_2fac$sample, Mean.Res_test_small_2fac$j),-c(8:23)],
                      Mean.Res_test_large_2fac[order(Mean.Res_test_large_2fac$scale, Mean.Res_test_large_2fac$sample, Mean.Res_test_large_2fac$j),-c(8:23)])


Mean.Res_2fac_NoARS <- rbind(Mean.Res_test_small_2fac[order(Mean.Res_test_small_2fac$scale, Mean.Res_test_small_2fac$sample, Mean.Res_test_small_2fac$j),-c(24:39)],
                        Mean.Res_test_large_2fac[order(Mean.Res_test_large_2fac$scale, Mean.Res_test_large_2fac$sample, Mean.Res_test_large_2fac$j),-c(24:39)])


####################################################################################################################################
# Null

Mean.Res_test_null.final <- Mean.Res_test_null[order(Mean.Res_test_null$Nfac, Mean.Res_test_null$scale, Mean.Res_test_null$sample, Mean.Res_test_null$j),c(1:7,8:23)]
...
write.csv(format(Conv.Res_NoArs.fin, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Conv.Scale_NoARS.csv", quote = F)
write.csv(format(Conv.Res_1fac_ARS, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Conv.Scale_ARS_1FAC.csv", quote = F)
write.csv(format(Conv.Res_2fac_ARS,nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Conv.Scale_ARS_2FAC.csv", quote = F)
write.csv(format(cbind(subset(Conv.Res_test_null.final, Conv.Res_test_null.final$Nfac==1), subset(Conv.Res_test_null.final, Conv.Res_test_null.final$Nfac==2)[,8:10])
                 , nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Conv.Scale_Null.csv", quote = F)
write.csv(format(Res_1fac_ARS, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Scale_ARS_1FAC.csv", quote = F)
write.csv(format(Res_2fac_ARS,nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Scale_ARS_2FAC.csv", quote = F)
write.csv(format(Res_1fac_NoARS, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Scale_NoARS_1FAC.csv", quote = F)
write.csv(format(Res_2fac_NoARS, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Scale_NoARS_2FAC.csv", quote = F)
write.csv(format(Res_test_null.final, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Scale_Null.csv", quote = F)

#Mean results
write.csv(format(Mean.Res_1fac_ARS, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Mean.Scale_ARS_1FAC.csv", quote = F)
write.csv(format(Mean.Res_2fac_ARS,nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Mean.Scale_ARS_2FAC.csv", quote = F)
write.csv(format(Mean.Res_1fac_NoARS, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Mean.Scale_NoARS_1FAC.csv", quote = F)
write.csv(format(Mean.Res_2fac_NoARS, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Mean.Scale_NoARS_2FAC.csv", quote = F)
write.csv(format(Mean.Res_test_null.final, nsmall=3), "C:/Users/eddurso/Desktop/Projects/D'Urso MI and RS/Response styles and MI/Scripts/Tables/DIFARS_Mean.Scale_Null.csv", quote = F)
