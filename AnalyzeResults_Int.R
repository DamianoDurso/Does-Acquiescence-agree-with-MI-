...
for(k in 1:nrow(simdes)){
  load(paste0("Int_ResultsSimScale_",k,".RData"))  
...
for(i in 1:100){
  ...
  if(Scale_level.models[[i]]$Comparative.fit$ARSModels$error[1] != "none" | Scale_level.models[[i]]$Comparative.fit$ARSModels[[10]] != "none"){
    ARS_Int_chi[i] <- NA# < .05
  } else {
...
# ARS Absolute fit
  ...
  Results.scale.mean$NoARS_Int_df[k] <- round(mean(NoARS_Int_df),1)
